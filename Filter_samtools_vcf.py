...
try:
	options, args = getopt.getopt(sys.argv[1:], 'SB:BB:EB:MB:VDB:ABS:ABI:d:D:QS:QI:Qual:w:W:c:sw:', [])
...
for (oflag, oarg) in options:
	...
	if oflag == '-c': snpcluster = int(oarg)
	if oflag == '-sw': snpclusterwindow = int(oarg)
...
tempfile = open("/scratch/"+str(args[0].split("/")[-1])+"_"+str(rand)+".tmp","w")
...
for line in input:
	...
	if vartype == "SNP":
		...
		if int(infodict["DP4"].split(",")[2])+int(infodict["DP4"].split(",")[3]) < minaltsnp:
                	tag = tag+"LOW NON-REF READS;"
		if float(int(infodict["DP4"].split(",")[0])+int(infodict["DP4"].split(",")[1]))/float(int(infodict["DP4"].split(",")[0])+int(infodict["DP4"].split(",")[1])+int(infodict["DP4"].split(",")[2])+int(infodict["DP4"].split(",")[3])) >= .20:
...
inputfile = open("/scratch/"+str(args[0].split("/")[-1])+"_"+str(rand)+".tmp", 'r').read()
input = []
...
for p in range(0,len(VCF)):
	...
	count = 0
        ...
        if not VCF[p][7].startswith("INDEL") and VCF[p][-1] == "PASS":
		count = 1
                for q in range(p+1,len(VCF)):
                        if VCF[q][-1] == "PASS" and not VCF[q][7].startswith("INDEL"):
				count = count + 1
			if count == snpcluster:
				if int(VCF[q][1]) > int(VCF[p][1]) + snpclusterwindow:
					...
					for z in range(p,q+1):
						if VCF[z][-1] == "PASS" and not VCF[z][7].startswith("INDEL"):
...
os.system("rm /scratch/"+str(args[0].split("/")[-1])+"_"+str(rand)+".tmp")
